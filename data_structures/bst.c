...
struct Node *_addNode(struct Node *cur, TYPE val)
{
	...
	// Otherwise, use comparison result to move left or right
...
TYPE _leftMost(struct Node *cur)
{
	...
	// Keep going left until null node reached
	while( cur->left!=0 ){
...
struct Node *_removeLeftMost(struct Node *cur)
{
	...
	// Otherwise, remove node, and return right child
	if( cur->left!=0 ){
...
struct Node *_removeNode(struct Node *cur, TYPE val)
{
	...
	// When the value is found, replace with left child if right child null
	// Else, replace with leftmost right child
	if( compare(cur->val,val) == 0 ){
